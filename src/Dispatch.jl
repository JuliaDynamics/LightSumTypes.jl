...
function define_f_super(mod, f_super_dict, f_cache)
    ...
    if !(f_name in keys(cache))
        ...
        never_same = true
        f_sig = Base.signature_type(mod.MixedStructTypes.inspect_sig, Base.eval.(mod, f_cache))
        for sig in keys(cache[f_name])
            k_sig = Base.signature_type(mod.MixedStructTypes.inspect_sig, Base.eval.(mod, sig))
